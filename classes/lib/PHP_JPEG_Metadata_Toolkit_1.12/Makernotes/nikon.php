...
function get_Nikon_Makernote( $Makernote_Tag, $EXIF_Array, $filehnd, $Make_Field ) {
	...
	} elseif ( ( substr( $Makernote_Tag['Data'], 0, 10 ) == "Nikon\x00\x02\x10\x00\x00" ) ||
	( substr( $Makernote_Tag['Data'], 0, 10 ) == "Nikon\x00\x02\x00\x00\x00" ) ) {
		...
		// Return the new tag
		return $Makernote_Tag;
	} elseif ( substr( $Makernote_Tag['Data'], 0, 8 ) == 'FUJIFILM' ) {
...
function get_Nikon_Text_Value( $Exif_Tag, $Tag_Definitions_Name ) {
	...
	if ( $Tag_Definitions_Name == 'Nikon Type 1' ) {
		...
		return false;
	} elseif ( $Tag_Definitions_Name == 'Nikon Type 3' ) {
		...
		} elseif ( ( $Exif_Tag['Tag Number'] == 2 ) || // ISO Speed Used
		( $Exif_Tag['Tag Number'] == 19 ) ) { // ISO Speed Requested
		...
		} elseif ( $Exif_Tag['Tag Number'] == 137 ) {  // Bracketing & Shooting Mode
			...
			// Return the output
			return $outputstr;
		} elseif ( $Exif_Tag['Tag Number'] == 136 ) {  // Auto Focus Area
