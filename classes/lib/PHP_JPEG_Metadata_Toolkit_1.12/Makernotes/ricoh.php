...
function get_Ricoh_Makernote( $Makernote_Tag, $EXIF_Array, $filehnd, $Make_Field ) {
	...
	elseif ( $Makernote_Tag['Data'] === str_repeat( "\x00", strlen( $Makernote_Tag['Data'] ) ) ) {
	...
	elseif ( ( substr( $Makernote_Tag['Data'], 0, 5 ) === 'RICOH' ) ||
...
function get_Ricoh_Makernote_Html( $Makernote_tag, $filename ) {
	...
	elseif ( $Makernote_tag['Makernote Type'] == 'Ricoh Text' ) {
	...
	elseif ( $Makernote_tag['Makernote Type'] == 'Ricoh Empty Makernote' ) {
		// Do Nothing
		return '';
	} else {
