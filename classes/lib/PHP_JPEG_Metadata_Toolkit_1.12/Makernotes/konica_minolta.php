...
function get_Minolta_Makernote( $Makernote_Tag, $EXIF_Array, $filehnd,
								$Make_Field ) {
	...
	} elseif ( substr( $Makernote_Tag['Data'], 0, 2 ) == 'KC' ) {
	...
	if ( substr( $Makernote_Tag['Data'], 0, 8 ) == '+M+M+M+M' ) {
		...
		return $Makernote_Tag;
	} elseif ( substr( $Makernote_Tag['Data'], 0, 5 ) == 'MINOL' ) {
...
function get_Minolta_Text_Value( $Exif_Tag, $Tag_Definitions_Name ) {
	...
	( $Exif_Tag['Tag Number'] == 0x0003 ) ) {
		...
		return $output_str;
	} elseif ( ( $Exif_Tag['Tag Number'] == 0x0088 ) ||
