...
class File extends Model {
	...
	public function add_attachment( $post_ID ) {
		...
		if ( 'image/jpeg' == $mimeType || 'image/tiff' == $mimeType ) {
			if ( function_exists( 'exif_read_data' ) ) {
				$metaData = exif_read_data( $image, 'FILE', true );
				if ( method_exists( $this( 'image_metadata' ), 'get_img_meta_data' ) ) {
					$imageMeta = $this( 'image_metadata' )->get_img_meta_data( $image );
				} else {
					$imageMeta = false;
				}
			}
	...
	public function get_plugin_version( $name ) {
		...
		if ( is_array( $name ) ) {
			...
			$filePath		 = ABSPATH . 'wp-content/plugins/' . $name . '/' . $name . '.php';
			$pluginObject	 = get_plugin_data( $filePath );
