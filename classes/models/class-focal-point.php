...
class Focal_point extends Model {
	...
	public function post_focal_point( $params ) {
		...
		$new_point		 = $this->calculate_focal_point( $params['id'], $point, $size );
		$params['xPos']	 = $new_point->X;
		$params['yPos']	 = $new_point->Y;
		// delate all amages
		$this( 'image' )->delete_thumbnails( $params['id'] );
		// update meta data
		$this->update_post_meta( $params['id'], 'focal_point', $params );
		$filePath		 = get_attached_file( $params['id'] );
		// save new copy img
		global $_wp_additional_image_sizes;
