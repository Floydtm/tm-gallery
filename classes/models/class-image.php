...
class Image extends Model {
  ...
  $point                = new Point( $focal_point['xPos'], $focal_point['yPos'] );
	 ...
	 * @param type $params
	 */
	public function optimaze_image( $params ) {
		$attachments = get_posts( $this( 'media' )->get_content_params( array( 'fields' => 'ids' ) ) );
		$ng_images	 = $this->get_next_get_images();
		$posts		 = array_merge( $ng_images, $attachments );
		// activate plugin if empty attachments
		if ( empty( $posts ) ) {
			update_option( 'tm_pg_first_activated', 0 );
			$params['id'] = 0;
		}
		$image	 = get_post( $params['id'] );
		$success = true;
		if ( 'ngg_pictures' == $image->post_type ) { // check if next gallery post
			$parent = get_posts(
			array(
			...
			// create attachmet if not created
			if ( empty( $parent ) ) {
				$id = $this->import_Next_Gen_image( $params['id'] );
				if ( $id ) {
					$params['id'] = $id;
				} else {
					$success = false;
		...
		if ( $success ) {
			foreach ( self::get_sizes_by_type( 'img' ) as $size => $value ) {
				if ( $value ) {
					image_downsize( $params['id'], $size );
				}
			}
		}
		...
		return $this( 'model' )->get_arr( $params, true );
	}
	 ...
	 * @return boolean
	 */
	public function get_next_get_images() {
		// get next get posts
		$images = get_posts( array(
			'post_type'		 => 'ngg_pictures',
			'post_status'	 => 'any',
			'fields'		 => 'ids',
			'posts_per_page' => -1,
		) );
	 ...
	 * @return boolean
	 */
	public function import_Next_Gen_image( $id ) {
		global $wpdb;
		$return	 = false;
		// get next gen image
		$ng_obj	 = $wpdb->get_results(
		$wpdb->prepare(
		"SELECT DISTINCT {$wpdb->prefix}ngg_pictures.image_slug, {$wpdb->prefix}ngg_pictures.filename,{$wpdb->prefix}ngg_gallery.path " .
		"FROM {$wpdb->prefix}ngg_pictures " .
		"LEFT JOIN {$wpdb->prefix}ngg_gallery ON {$wpdb->prefix}ngg_pictures.galleryid = {$wpdb->prefix}ngg_gallery.gid " .
		...
		if ( ! empty( $ng_obj ) ) {
			...
			if ( file_exists( $path ) ) {
				...
				$file_data	 = array(
					'error'		 => 0,
					'name'		 => $ng_obj->filename,
					'type'		 => finfo_file( $finfo, $path ),
					'size'		 => filesize( $path ),
					'tmp_name'	 => $path,
				);
			}
			$return = $this( 'file' )->upload_file( $file_data, $id );
		}
		return $return;
	}
}
