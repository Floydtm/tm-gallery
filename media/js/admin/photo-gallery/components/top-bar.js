					...
					_grid.deleteItem( id, function ( $data ) {
						...
						if ( !_.isUndefined( sets ) ) {
							_.each( sets, function ( set ) {
								_pgFolder.updateFolder( set, 'set' );
							} );
						}
						if ( !_.isUndefined( albums ) ) {
							_.each( albums, function ( album ) {
								_pgFolder.updateFolder( album, 'album' );
							} );
						}
						...
						if ( _pgFolder._ID ) {
							...
							// toggle select all in folder
							_.each( _pgFolder._types, function ( type ) {
								if ( !_pgFolder.getLength( type ) ) {
									_grid.toggleSelectAllBtn( type );
								}
							} );
						}
