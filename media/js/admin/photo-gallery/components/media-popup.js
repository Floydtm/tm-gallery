        ...
        return {
            ...
            addToFolder: function ( ids, key ) {
                ...
                __( 'common' ).wpAjax( $params, function ( ) {
                    var $item = _content.getContent( ids[key], 'img' );
                    if ( !_.isUndefined( $item ) ) {
                        __( type ).addRightCallback( ids, key, id );
                    }
                    if ( !_.isUndefined( ids[++key] ) ) {
                        state.addToFolder( ids, key );
                    } else {
                        var status = 'allready_added',
                            images = _folder._folder.childs.img;
                        ...
                        _folder.updateFolder( id, type, function () {
                            ...
                            __( 'notification' ).show( status + '_to_' + type, { name: title } );
                        } );
                    }
                } );
            }
