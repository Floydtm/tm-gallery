		...
		return {
			...
			init: function ( ids ) {
				...
				state.showSlidebar( state.ids, function () {
					...
					$( state._slidebar + " .select2" ).select2();
				} );
			},
			/**
			 * Refresh slidebar
				...
				try {
					...
					switch ( type ) {
						case 'img':
							...
							// set img src
							$( selector + ' .tm-pg_sidebar_image img' ).attr( 'src', content.thumbnails.right.url );
						...
						case 'set':
							if ( !_.isUndefined( content.cover_img ) && !_.isEmpty( content.cover_img.right ) ) {
								// set img src
								$( selector + ' .tm-pg_sidebar_image img' ).attr( 'src', content.cover_img.right[0] );
								// show img
								$( selector + ' .tm-pg_sidebar_image_container' ).removeClass( 'hidden' );
							}
							break;
					...
					$( selector + ' .tm-pg_sidebar_image_main-description input[name="post_content"]' ).val( content.post.post_content );
					// show existing tags
